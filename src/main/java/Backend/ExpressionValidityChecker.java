...
public class ExpressionValidityChecker implements PropertyChangeListener {
    ...
    public void preCheck(List<String> terms) throws InvalidTermException {
        if (terms.size() == 0) {
            ...
            return; //This is intentional to consider the case where we have a single term.
        } else if (!checkMatchingBrackets(terms)) { //Check if each bracket has a corresponding bracket and if we don't
                                                    // have any extra brackets.
