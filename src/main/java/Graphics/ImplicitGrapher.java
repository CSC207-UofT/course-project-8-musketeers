...
public class ImplicitGrapher {
   ...
   public void graphImplicit(int[] pixels, int w, int h, Axes ax,
  ...
  public void graphImplicit(int[] pixels, int w, int h, Axes ax,
      ...
      for (int y = 0; y < h; y++) {
         for (int x = 0; x < w; x++) {
            writePixel(pixels, w, h, ax, gtype, func, x, y);
         }
      }
  }

  private void writePixel(int[] pixels, int w, int h, Axes ax,
                          GraphType gtype,
                          Evaluatable func, int x, int y) {
